...
const AdminDashboard = {
  ...
  async loadStats() {
    ...
    document.getElementById('statTotal')   && (document.getElementById('statTotal').textContent   = d.total_employees   ?? '—');
    document.getElementById('statHighRisk')&& (document.getElementById('statHighRisk').textContent= d.high_risk_count    ?? '—');
    document.getElementById('statSessions')&& (document.getElementById('statSessions').textContent= d.active_sessions    ?? '—');
  },
  ...
  renderTable() {
    ...
    if (!this.filtered.length) {
      tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted" style="padding:2rem">No employees found</td></tr>';
      return;
    }

    tbody.innerHTML = this.filtered.map(emp => `
      <tr>
        <td>
  ...
  async block(empId, name) {
    const ok = await UI.confirm(`Block ${name}? Their sessions will be terminated.`, 'Block Employee');
    if (!ok) return;
  ...
  async forceLogout(empId, name) {
    const ok = await UI.confirm(`Force logout ${name}? All sessions will be terminated.`, 'Force Logout');
    if (!ok) return;
...
const AdminEmployeeProfile = {
  ...
  renderProfile(p) {
    ...
    // Header
    document.getElementById('empName')   && (document.getElementById('empName').textContent   = p.name);
    document.getElementById('empEmail')  && (document.getElementById('empEmail').textContent  = p.email);
    document.getElementById('empDept')   && (document.getElementById('empDept').textContent   = `${p.department} · ${p.job_title}`);
    document.getElementById('empStatus') && (document.getElementById('empStatus').innerHTML   =
      p.is_blocked
        ? '<span class="badge badge-blocked">Blocked</span>'
        : '<span class="badge badge-active">Active</span>');
    document.getElementById('empAvatarLetter') && (document.getElementById('empAvatarLetter').textContent = (p.name || 'U')[0].toUpperCase());
  ...
  renderLinkClicks(clicks) {
    ...
    if (!el) return;
    if (!clicks.length) {
      el.innerHTML = '<p class="text-muted">No phishing link clicks recorded</p>';
      return;
    }
    el.innerHTML = clicks.map(c => {
      const ms = c.metadata?.response_time_ms;
      const speed = ms != null
        ? (ms < 3000 ? '🔴 Very fast' : ms < 10000 ? '🟡 Moderate' : '🟢 Slow')
        : '—';
      return `
        <div class="task-item">
  ...
  async block(empId, name) {
    ...
    if (!ok) return;
    const res = await API.post(`/admin/employee/${empId}/block`, {});
    if (res && res.ok) { Toast.success('Employee blocked'); location.reload(); }
  ...
  async unblock(empId, name) {
    ...
    if (!ok) return;
    const res = await API.post(`/admin/employee/${empId}/unblock`, {});
    if (res && res.ok) { Toast.success('Employee unblocked'); location.reload(); }
    else Toast.error(res?.data?.error || 'Failed');
  },
...
const AdminReports = {
  ...
  downloadReport() {
    ...
    const blob = new Blob([JSON.stringify(this._lastReport, null, 2)], { type: 'application/json' });
